function [L, dLdSPD] = cholj(SPD)
% Returns lower cholesky decomposition s.t. SPD = L*L' and its derivative
%In:
%    SPD    D x D x N
% Out
%    L      D x D x N
%   dLdSPD  D x D x N x D x D x N
%
% Test code:
%{
clear, clc, rng default; addpath('./mtools');
D = 2; N=3; A = rand(D); SPD = bsxfun(@times,A*A',rand(1,1,N));
[r, num, ana] = checkGrad(@cholj,1,1,2,{SPD});
%}
[D,E,N] = size(SPD);  if D~=E, error('is not square');end
L = zeros(D,D,N);
if nargout ==1,
    for n=1:N
        L(:,:,n) = chol(SPD(:,:,n),'lower');
    end
else
   dLdSPD = zeros(D,D,N,D,D,N);

    triD = (D+1)*D/2; iiL = triu(true(D));ii = find(iiL);
    for n=1:N
        dir = zeros(D,D,triD);
        for i = 1:triD
            [m,l] = ind2sub([D D],ii(i));
            dir(m,l,i)=1;dir(l,m,i)=1;
        end
        
        [U, dUdSPD] = derivchol(SPD(:,:,n), dir);
        
        for i = 1:length(ii)
            [m,l] = ind2sub([D D],ii(i));
            dLdSPD(:,:,n,m,l,n) = dUdSPD(:,:,i)';
            % dLdSPD(:,:,l,m) = dUdSPD(:,:,i)';
        end
