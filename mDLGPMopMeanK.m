classdef mDLGPMopMeanK <handle
    ...
    properties%(Access = protected)
        ...
        sigmaFf ; %sigma_F of f
        sigmaFg ; %sigma_F of g
        lengthSf ; %lenght-scale of f
        lengthSg ; %lenght-scale of g
        sigmaN; %sigma_N 
    ...
    methods
        function obj = init(obj, in_xSize,in_pts,in_N)
            ...
            obj.sigmaFf = ones(obj.outs,obj.N);
            obj.sigmaFg = ones(obj.outs,obj.N);
            
            if obj.ard == 1
                obj.lengthSf = ones(obj.xSize*obj.outs,obj.N);
        ...
        function kern = kernelF(obj, Xi, Xj,out, model)%squared exponential kernel
        ...
        function kern = kernelG(obj, Xi, Xj,out, model)%squared exponential kernel
            kern = (obj.sigmaF(out,obj.auxUbic(model))^2)*...
                exp(-0.5*sum(((Xi-Xj).^2)./(obj.lengthS((out-1)*obj.amountL+1:out*obj.amountL,obj.auxUbic(model)).^2),1))';
        end
        ...
        function [ df] = grad(obj,hyps,model,p)
            ...
            obj.alpha(p*obj.pts+1:p*obj.pts+obj.localCount(model),obj.auxUbic(model)) = ...
                ...
                (obj.auxUbic(model)-1)*obj.pts+obj.localCount(model))' -...
                obj.meanFunction{p+1}(obj.X(:,(obj.auxUbic(model)-1)*obj.pts+1:...
                (obj.auxUbic(model)-1)*obj.pts+obj.localCount(model)))' );
        ...
        function updateParam(obj,x,model)
            if obj.localCount(model) == 1 %first point in model
                for p = 0:obj.outs-1
                    ...
                    obj.alpha(p*obj.pts+1,pos+1) = obj.K(p*obj.pts+1,(pos)*obj.pts+1)\...
                        (obj.Y(p+1,(pos)*obj.pts+1) - obj.meanFunction{p+1}(x));
                end
            end
